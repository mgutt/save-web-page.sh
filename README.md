# wgetg
`wgetg` is an alternative to `wget` or `curl`, which opens an URL in a graphical browser and returns it html source or saves the web page as html file by using `xdotool`. By that the html source code contains interactive changes made by javascript.
